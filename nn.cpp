...
Matrix *NN::get_output() { return &activations[activations.size() - 1]; }
...
void NN::nn_step(NN &grad_nn, float lr) {
  ...
  for (size_t l = 0; l < weights.size(); l++) {
    ...
    launch_matrix_gradient_step_kernel(w, g_w, lr, w_shape.rows, w_shape.cols);
    launch_matrix_gradient_step_kernel(b, g_b, lr, b_shape.rows, b_shape.cols);
  }
}
